//
// Created by Diogo Cecilio on 5/07/22.
//

#ifndef TPZDARCYFLOWISOPERM_H
#define TPZDARCYFLOWISOPERM_H
 ...
 *
 * The Darcy flow equation is given by: div( K grad(p) ) = 0
 *K={{k11,0,0},{0,k22,0},{0,0,k33}} 
 */

class TPZDarcyFlowIsoPerm : public  TPZMaterial {
...
public:
	 ...
	 */
    TPZDarcyFlowIsoPerm(int id, int dim);

    /**
	 * @brief Returns a 'std::string' with the name of the material
	 */
    virtual std::string Name()   const{ return "TPZDarcyFlowIsoPerm"; }
   ...
   // [[nodiscard]] TPZMaterial *NewMaterial() const ;
	...
	void SetConstantPermeability(TPZManVector<REAL,3> perm)
	{
...
protected:
	...
	TPZManVector<REAL,3> fConstPermeability;
};
