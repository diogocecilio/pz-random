...
public:
  ...
  TPZPlasticStepVoigt(REAL alpha=0.):fYC(), fER(), fResTol(1.e-12), fMaxNewton(30), fN()
	{ 
		fN.fAlpha = alpha; 
	}
	 ...
	 */
	TPZPlasticStepVoigt(const TPZPlasticStepVoigt & source)
	{
		fYC			= source.fYC;
		fER			= source.fER;
	 ...
	 * @param[in] source of copy
	 */
	TPZPlasticStepVoigt & operator=(const TPZPlasticStepVoigt & source)
	{
		fYC			= source.fYC;
		fER			= source.fER;
		fResTol		= source.fResTol;
