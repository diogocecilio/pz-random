...
void MonteCarlo2 ( int a,int b )
{
    ...
    //TPZGeoMesh *gmeshfield1 = CreateGMesh ( ref,file );
    //TPZGeoMesh *gmeshfield2 = CreateGMesh ( ref,file );
    TPZGeoMesh *gmeshfield = TriGMesh ( ref,"m1" );
    TPZGeoMesh *gmeshfield1 = TriGMesh ( ref,"m2"  );
    TPZGeoMesh *gmeshfield2 = TriGMesh ( ref,"m3"  );
...
TPZGeoMesh * TriGMesh(int ref,string file)
{
        ...
        for ( int iel=0; iel<topol.size(); iel++ ) {
                if(topol[iel].size()==3)
                {
                        ...
                        REAL x0 = co[TopoLine[0]][1];
                        REAL y0 = co[TopoLine[0]][2];
                        REAL xf = co[TopoLine[1]][1];
                        REAL yf = co[TopoLine[1]][2];
                        REAL tol=1.e-3;
                        REAL L=70;
                        REAL h1=30;
                        REAL h2=10;
