...
//  // B.Print ( std::cout );
// }
void KLStrMatrix::GetDestIndex ( int iel, TPZElementMatrix *elmat, TPZManVector<long> &fSourceIndex, TPZManVector<long> &fDestinationIndex )
{
        if (!elmat->HasDependency()){
		elmat->fSourceIndex.Resize(elmat->fMat.Rows());
		elmat->fDestinationIndex.Resize(elmat->fMat.Rows());
		long destindex = 0L;
        long fullmatindex = 0L;
		//const int numnod = elmat->NConnects();
		const int numnod = fMesh->ElementVec() [iel]->NConnects();
		for(int in = 0; in < numnod; in++){
			//const long npindex = elmat->ConnectIndex(in);
			const long npindex = fMesh->ElementVec() [iel]->ConnectIndex ( in );
			TPZConnect &np = elmat->fMesh->ConnectVec()[npindex];
			long blocknumber = np.SequenceNumber();
			long firsteq = elmat->fMesh->Block().Position(blocknumber);
			int ndf = elmat->fMesh->Block().Size(blocknumber);
            if(np.HasDependency() || np.IsCondensed()) {
                fullmatindex += ndf;
                continue;
            }//for (np)
			for(int idf=0; idf<ndf; idf++){
				elmat->fSourceIndex[destindex] = fullmatindex++;
				elmat->fDestinationIndex[destindex++] = firsteq+idf;
			}//for idf
		}//for in
        elmat->fSourceIndex.Resize(destindex);
        elmat->fDestinationIndex.Resize(destindex);		
	}//if
	else{
    
        long destindex = 0L;
        long fullmatindex = 0L;
        elmat->fDestinationIndex.Resize(elmat->fConstrMat.Rows());
        elmat->fSourceIndex.Resize(elmat->fConstrMat.Rows());
        //int numnod = elmat->fConstrConnect.NElements();
		const int numnod = fMesh->ElementVec() [iel]->NConnects();
        for(int in = 0; in < numnod; in++){
            //const long npindex = elmat->fConstrConnect[in];
			const long npindex = fMesh->ElementVec() [iel]->ConnectIndex ( in );
